...
static void generate_message_secret(dsa_params* params, huge* k) {
    ...
    unsigned char *a;
    int size = hex_decode((unsigned char*) "0x49fb8d96c64584d71bdfba03e56b62d3155e27eb", &a);
    huge_load(k, a, size);
    printf("k:");
    show_hex(k->rep, k->size, HUGE_WORD_BYTES);
}
...
void dsa_sign(
    ...
    hash_size = hash_size > params->q.size * HUGE_WORD_BYTES ? params->q.size * HUGE_WORD_BYTES : hash_size;
    huge_load(&m, hash, hash_size);
    printf("m:");
    show_hex(m.rep, m.size, HUGE_WORD_BYTES);
    huge_inverse_mul(&k, &params->q);
    printf("invk:");
    show_hex(k.rep, k.size, HUGE_WORD_BYTES);
    huge_copy(&s, private_key);
    printf("x:");
    show_hex(s.rep, s.size, HUGE_WORD_BYTES);
    huge_multiply(&s, &r);
    printf("xr:");
    show_hex(s.rep, s.size, HUGE_WORD_BYTES);
    huge_add(&s, &m);
...
int main() {
    ...
    int result = dsa_verify(&params, &x, &ctx, &signature);
    printf("dsa_verify:%d\n", result);
}
#endif